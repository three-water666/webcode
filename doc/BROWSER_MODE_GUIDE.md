...
- 独立 profile 默认储存在本机应用数据目录，不随 VS Code 插件卸载自动删除：
...
- 如需自定义储存位置，可以设置 `webcodeGateway.isolatedBrowser.profileRoot`；webcode 会在该目录下创建 `edge` 和 `chrome` 子目录。
- `webcodeGateway.isolatedBrowser.profileRoot` 只是选择新的 profile 根目录，不会迁移已有 profile。建议在第一次使用独立模式、也就是启动独立 Edge/Chrome 之前配置；这样只会创建一套 profile。
- 如果已经启动过独立模式后再修改 `webcodeGateway.isolatedBrowser.profileRoot`，webcode 会在新位置再创建一套独立 profile，原位置的 profile 会保留。切换回原配置时，只要原 profile 还在，对应登录态也还在。
- 如果因此产生了多套 profile，可以在 webcode 菜单中使用 `重置当前独立 profile` 删除当前配置正在使用的新目录；旧版 VS Code 插件储存目录则使用 `清理旧版独立 profile` 删除。
- 从旧版本升级后，webcode 不会复制旧的 VS Code 插件储存目录，而是直接使用新的独立 profile。你需要在新 profile 中重新登录一次 AI 站点。
- 如果检测到旧版独立 profile，webcode 菜单会显示 `清理旧版独立 profile`，用于删除旧 VS Code 插件储存目录里的遗留数据。
- `重置当前独立 profile` 会删除当前正在使用的新 profile。菜单里的重置/清理项右侧提供打开目录图标，删除失败时也可以跳转到目录手动处理。
- 如果彻底不再使用 webcode，可以先用上述菜单清理当前和旧版独立 profile，再卸载 VS Code 扩展。
...
- Chrome 独立 profile 与 Edge 独立 profile 使用同一个根目录下的 `chrome` 子目录。
