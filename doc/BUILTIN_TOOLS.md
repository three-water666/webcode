...
| `terminal_session` | 管理由 `run_in_terminal` 创建的终端会话：`action=list` 查看状态，`action=read` 读取输出并可用 `delay_seconds` 等待 0 到 10 秒后再读，`action=stop` 发送 `Ctrl+C` 中断当前命令并保留终端窗口，`action=close` 关闭终端标签页。 |
