...
| `run_in_terminal` | 在 VS Code 可见终端会话中启动单条命令，立即返回 `session_id`，适合长时间运行或需要用户可见输出的任务。 |
...
| 工具名 | 位置 | 作用 |
| --- | --- | --- |
| `execute_command` | `gateway-vscode/src/servers/command.ts` | 在后台执行单条短生命周期命令，要求命令在安全 allowlist 内、不能包含 shell 链接/管道/重定向/命令替换，并且 `cwd` 必须位于当前 workspace 内。 |
