...
收到用户的下一次回复后，先确认上一轮发出的每个工具调用都有对应 `request_id` 的结果；如果缺少某个 `request_id`，可能是工具调用未被 {{PRODUCT_NAME}} 成功捕获。读取类工具缺少结果时需要重新调用；写入类工具或命令缺少结果时，先确认操作是否真的没有执行，如果没有执行，也需要重新调用。重新调用时必须更换新的 `request_id`。
